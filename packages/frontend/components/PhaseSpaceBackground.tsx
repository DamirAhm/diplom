...
const PhaseSpaceBackground: React.FC<PhaseSpaceBackgroundProps> = ({
  ...
  const complexitySettings = {
    low: { points: 40, historyLength: 20, fadeSpeed: 0.03 },
    medium: { points: 60, historyLength: 30, fadeSpeed: 0.02 },
    high: { points: 100, historyLength: 40, fadeSpeed: 0.015 },
  };

  const getResponsiveSettings = (width: number) => {
    const baseSettings = complexitySettings[complexity];
    if (width <= 640) {
      // Mobile
      return {
        ...baseSettings,
        points: Math.floor(baseSettings.points * 0.6),
        historyLength: Math.floor(baseSettings.historyLength * 0.7),
      };
    } else if (width <= 1024) {
      // Tablet
      return {
        ...baseSettings,
        points: Math.floor(baseSettings.points * 0.8),
        historyLength: Math.floor(baseSettings.historyLength * 0.85),
      };
    }
    return baseSettings;
  };
    ...
    if (canvasRef.current) {
      ...
      const resizeCanvas = () => {
        ...
        if (canvas.width !== width || canvas.height !== height) {
          ...
          settings.points = responsiveSettings.points;
          settings.historyLength = responsiveSettings.historyLength;
          initializePoints();
        }
      };
        ...
        for (let i = 0; i < settings.points; i++) {
          pointsRef.current.push({
            ...
            colorIndex: Math.floor(
              Math.random() * colorSchemes[colorScheme].length
            ),
          });
        }
      };
      ...
      const animate = (currentTime: number) => {
        ...
        pointsRef.current.forEach((point, idx) => {
          ...
          const scaleFactor =
            width <= 640
              ? SIMULATION_SETTINGS.mobileScaleFactor
              : width <= 1024
              ? SIMULATION_SETTINGS.tabletScaleFactor
              : 1;

          point.x += dx * PARAMS.scale * deltaTime * scaleFactor;
          point.y += dy * PARAMS.scale * deltaTime * scaleFactor;
