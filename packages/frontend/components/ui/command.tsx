...
const Command = ({
    ...
    <CommandPrimitive
        className={cn(
            "flex h-full w-full flex-col overflow-hidden rounded-[5px] bg-white text-slate-950 ",
            className,
        )}
        {...props}
    />
...
const CommandInput = ({
    ...
    <div
        className="flex items-center rounded-[5px] border px-3"
        cmdk-input-wrapper=""
    >
        <Search className="mr-2 h-4 w-4 shrink-0 opacity-50" />
        <CommandPrimitive.Input
            className={cn(
                "flex h-11 w-full rounded-[5px] bg-transparent py-3 text-sm outline-none placeholder:text-slate-500 disabled:cursor-not-allowed disabled:opacity-50 ",
                className,
            )}
            {...props}
        />
...
const CommandItem = ({
    ...
    <CommandPrimitive.Item
        className={cn(
            "relative flex cursor-pointer select-none items-center rounded-[5px] px-2 py-1.5 text-sm outline-none aria-selected:bg-slate-100 aria-selected:text-slate-900 data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50",
            className,
        )}
        {...props}
    />
