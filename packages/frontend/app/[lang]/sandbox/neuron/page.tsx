...
export default function Neuron({
    ...
    // Neuron parameters
    const [capacitance, setCapacitance] = useState<number>(22);
    const [tuningVoltage, setTuningVoltage] = useState<number>(0);
    const [modVoltage, setModVoltage] = useState<number>(0.04);
    ...
    const [signalAmplitude, setSignalAmplitude] = useState<number>(21);
    const [signalFrequency, setSignalFrequency] = useState<number>(1000);
    const [signalOffset, setSignalOffset] = useState<number>(0);

    // Simulation settings
    const [simTime, setSimTime] = useState<number>(30);
    ...
    const [parameterMapData, setParameterMapData] = useState<ParameterMapData | null>(null);
    ...
    const applyPreset = (preset: keyof typeof presets) => {
        ...
        setCapacitance(config.capacitance);
        setTuningVoltage(config.tuningVoltage);
        setModVoltage(config.modVoltage);
        ...
        if ('signalAmplitude' in config) {
            setSignalAmplitude(config.signalAmplitude as number);
        }
        if ('signalFrequency' in config) {
            setSignalFrequency(config.signalFrequency as number);
        }
        if ('signalOffset' in config) {
            setSignalOffset(config.signalOffset as number);
        }
    };
    ...
    const prepareTimeSeriesData = (): ChartData<'line'> => {
        ...
        console.log("Preparing time series data:", timeSeriesData);

        // Make sure we have valid data for plotting
        const validData = timeSeriesData.filter(point =>
        ...
        return {
            labels: validData.map(point => (point.t * 1000).toFixed(2)), // Convert to ms
            datasets: [
                {
                    label: 'Membrane Potential (V)',
                    data: validData.map(point => point.v),
                ...
                },
                {
                    ...
                    data: validData.map(point => point.I ? point.I * 1e6 : 0), // Convert to μA
    ...
    const prepareExcitabilityData = (): ChartData<'line'> => {
        ...
        return {
            labels: excitabilityData.currents.map((c: number) => (c * 1e6).toFixed(1)), // Convert to μA
            datasets: [
                {
    ...
    const prepareParameterMapData = (): { data: number[][], xLabels: string[], yLabels: string[], mapType: string } => {
        ...
        return {
            ...
            yLabels: parameterMapData.yValues?.map((y: number) =>
                parameterMapData.mapType === "ETDvsC"
                    ? y.toFixed(1)
                    : y.toFixed(3)
            ) || [],
    ...
    const timeSeriesOptions = useMemo(() => {
        ...
        let yMin = -0.1, yMax = 0.3;  // Default voltage range
        let y1Min = 0, y1Max = 100;   // Default current range in μA
        ...
        if (timeSeriesData.length > 0) {
            ...
            const currents = timeSeriesData.map(point => point.I ? point.I * 1e6 : 0);
    ...
    const phaseSpaceOptions = useMemo(() => {
        // Calculate min/max values for better scaling if we have data
        let xMin = -0.1, xMax = 0.3;  // Default voltage range
        let yMin = 0, yMax = 1;       // Default memristor state range
    ...
    const excitabilityOptions = useMemo(() => {
        // Calculate min/max values for better scaling if we have data
        let yMin = 0, yMax = 500;  // Default frequency range
    ...
    const ParameterMapRenderer = ({
        ...
        return (
            <div className="w-full h-[400px] p-4">
                <div className="text-center mb-4">Parameter Map: Excitability Classes</div>
                <div className="grid grid-cols-[auto_1fr] gap-2">
                    <div className="w-[50px] h-[300px] relative">
                        ...
                        <div className="absolute transform -rotate-90 origin-left top-1/2 -translate-y-1/2 -left-8 text-sm">
                            {preparedData.mapType === "ETDvsC" ? "Capacitance (nF)" : "E_VM (V)"}
                        </div>
                    </div>
                    <div>
                        <div className="h-[300px] bg-white relative">
                            {parameterMapData.classes?.map((row: number[], rowIdx: number) => (
                        ...
                        <div className="flex justify-between mt-2">
                            {preparedData.xLabels.map((label, idx) => (
                                idx % 3 === 0 ? (
                                    <div key={idx} className="text-xs">{label}</div>
                                ) : null
                            ))}
                        </div>
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <Card className="border-border bg-card">
                        <CardContent className="p-4 sm:p-6">
                            <div className="space-y-6">
                                <div>
                                    <h3 className="text-lg font-semibold mb-3">Neuron Parameters</h3>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            <div className="flex justify-between text-sm text-muted-foreground mb-2">
                                                <Label htmlFor="tuningVoltage">Tuning Voltage (V)</Label>
                                        ...
                                        <div>
                                            <div className="flex justify-between text-sm text-muted-foreground mb-2">
                                                <Label htmlFor="modVoltage">Modulatory Voltage (V)</Label>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <Label htmlFor="invertMemristor" className="text-sm text-muted-foreground">
                                                Invert Memristor
                                            </Label>
                                            <Switch
                                                id="invertMemristor"
                                ...
                                <div>
                                    ...
                                    {signalType !== "custom" && (
                                        <div className="space-y-4">
                                            <div>
                                                <div className="flex justify-between text-sm text-muted-foreground mb-2">
                                                    <Label htmlFor="signalAmplitude">Amplitude (μA)</Label>
                                                ...
                                                <Slider
                                                    ...
                                                    onValueChange={(value) => setSignalAmplitude(value[0])}
                                                />
                                            </div>
                                            ...
                                            {(signalType === "sine" || signalType === "pulse") && (
                                                <div>
                                                    <div className="flex justify-between text-sm text-muted-foreground mb-2">
                                                        <Label htmlFor="signalFrequency">Frequency (Hz)</Label>
                                            ...
                                            {(signalType === "sine" || signalType === "step") && (
                                                <div>
                                                    <div className="flex justify-between text-sm text-muted-foreground mb-2">
                                                        <Label htmlFor="signalOffset">Offset/Initial Value (μA)</Label>
                                    ...
                                    {signalType === "custom" && (
                                        <div>
                                            <Label htmlFor="customSignal" className="text-sm text-muted-foreground mb-2 block">
                                                Upload Custom Signal
                                            </Label>
                                            <Input
                                                id="customSignal"
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <Button variant="outline" onClick={() => applyPreset("class1")} size="sm">
                                            Class 1 Excitability
                                        </Button>
                                        <Button variant="outline" onClick={() => applyPreset("class2")} size="sm">
                                            Class 2 Excitability
                                        </Button>
                                        <Button variant="outline" onClick={() => applyPreset("class3")} size="sm">
                                            Class 3 Excitability
                                        </Button>
                                        <Button variant="outline" onClick={() => applyPreset("chaos")} size="sm">
                                            Chaotic Mode
                                        </Button>
                                    </div>
                                </div>
                ...
                <div className="lg:col-span-3">
                        ...
                        <CardContent className="p-4 sm:p-6">
                            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                                <TabsList className="mb-4 w-full bg-background border-b border-border">
                                    <TabsTrigger value="time-series" className="data-[state=active]:bg-background">Time Series</TabsTrigger>
                                    <TabsTrigger value="excitability" className="data-[state=active]:bg-background">Excitability Test</TabsTrigger>
                                    <TabsTrigger value="parameter-map" className="data-[state=active]:bg-background">Parameter Maps</TabsTrigger>
                                </TabsList>
                                ...
                                <TabsContent value="time-series">
                                    <div className="min-h-[400px] flex items-center justify-center bg-muted rounded-lg border border-dashed border-border">
                                        {timeSeriesData.length > 0 ? (
                                            ...
                                            <div className="text-muted-foreground">Run a simulation to see results</div>
                                        )}
                                    </div>
                                    <div className="mt-4 min-h-[200px] flex items-center justify-center bg-muted rounded-lg border border-dashed border-border">
                                        {timeSeriesData.length > 0 ? (
                                            <div className="w-full h-[200px]">
                                                <Scatter data={preparePhaseSpaceData()} options={phaseSpaceOptions} />
                                            </div>
                                        ) : (
                                            <div className="text-muted-foreground">Run a simulation to see phase space</div>
                                        )}
                                    </div>
                                </TabsContent>
                                ...
                                <TabsContent value="excitability">
                                    <div className="min-h-[400px] flex items-center justify-center bg-muted rounded-lg border border-dashed border-border">
                                        {excitabilityData ? (
                                            <div className="w-full h-[400px]">
                                                <Line data={prepareExcitabilityData()} options={excitabilityOptions} />
                                            </div>
                                        ) : (
                                            <div className="text-muted-foreground">Run a simulation to see excitability test results</div>
                                        )}
                                    </div>
                                </TabsContent>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                ...
                                                if (parameterMapData && activeTab === "parameter-map") {
                                                    console.log("Changing map type and running new simulation...");
                                                    runSimulation();
                                                }
                                            }}
                                        >
                                            ...
                                            <SelectContent>
                                                <SelectItem value="ETDvsC">E_TD vs Capacitance</SelectItem>
                                    ...
                                    <div className="min-h-[400px] flex items-center justify-center bg-muted rounded-lg border border-dashed border-border">
                                        {parameterMapData ? (
                                            <ParameterMapRenderer
                                                key={`map-${mapType}-${JSON.stringify(parameterMapData.xValues || [])}`}
                                                parameterMapData={parameterMapData}
                                                mapType={mapType}
                                            />
                                        ) : (
                                            <div className="text-muted-foreground">Run a parameter map simulation to see results</div>
                                        )}
                                    </div>
                                </TabsContent>
                            </Tabs>
                    ...
                    <div className="mt-6">
                        <h3 className="text-xl font-semibold mb-4 text-foreground">About Tunnel Diode Neurons</h3>
                        <p className="text-muted-foreground mb-4">
                            Tunnel diode neurons are electronic circuits that mimic the behavior of biological neurons using tunnel diodes and memristors.
                            They exhibit various excitability classes similar to those observed in real neurons, making them useful for neuromorphic computing.
                        </p>
                        <p className="text-muted-foreground mb-4">
                            The excitability classes correspond to different dynamic behaviors:
                        </p>
                        <ul className="list-disc pl-6 space-y-2 text-muted-foreground mb-4">
                            <li><strong>Class 1:</strong> Neurons that can fire at arbitrarily low frequencies</li>
                            <li><strong>Class 2:</strong> Neurons that fire in a limited frequency band</li>
                            <li><strong>Class 3:</strong> Neurons that fire only once at the onset of stimulation</li>
                        </ul>
                        <p className="text-muted-foreground">
                            This sandbox allows you to explore how different parameters affect the neuron's dynamics and response to various input signals.
                        </p>
                    </div>
                </div>
            </div>
